...
def write_version_py():
    with open(os.path.join("palm", "version.txt")) as f:
    ...
    with open(os.path.join("palm", "version.py"), "w") as f:
        f.write('__version__ = "{}"\n'.format(version))
    return version
...
if "clean" in sys.argv[1:]:
    ...
    subprocess.run(
        ["rm -f palm/*.so palm/**/*.so palm/*.pyd palm/**/*.pyd"],
        shell=True,
    )


extra_packages = []
if os.path.exists(os.path.join("palm", "model_parallel", "megatron", "mpu")):
    extra_packages.append("palm.model_parallel.megatron.mpu")
...
def get_files(path, relative_to="palm"):
    all_files = []
...
if __name__ == "__main__":
    try:
        ...
        fairseq_examples = os.path.join("palm", "examples")
        ...
        package_data = {
            "palm": (
                ...
                + get_files(os.path.join("palm", "config"))
            )
        }

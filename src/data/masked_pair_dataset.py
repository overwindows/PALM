    ...
    def __init__(
        ...
        if num_buckets > 0:
            ...
            if self.tgt is not None:
                ...
                logger.info(
                    "bucketing target lengths: {}".format(
                        list(self.tgt.buckets))
                )

            self.masked_src = BucketPadLengthDataset(
                self.masked_src,
                sizes=self.masked_src_sizes,
                num_buckets=num_buckets,
                pad_idx=self.src_dict.pad(),
                left_pad=self.left_pad_target,
            )
        ...
        if self.src_lang_id is not None or self.tgt_lang_id is not None:
            ...
            if self.src_lang_id is not None:
                res["net_input"]["src_lang_id"] = (
                    torch.LongTensor([[self.src_lang_id]]).expand(
                        bsz, 1).to(src_tokens)
                )
            if self.tgt_lang_id is not None:
                res["tgt_lang_id"] = (
                    torch.LongTensor([[self.tgt_lang_id]]).expand(
                        bsz, 1).to(src_tokens)
                )
        return res
